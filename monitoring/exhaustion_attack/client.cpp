...
parse(int argc, char* argv[])
{
  ...
  try
  {
    ...
    options.add_options()
      ...
      ("sleep1", "amount of time before start sending requests (sec)", cxxopts::value<uint32_t>()->default_value(std::to_string(1)), "N")
      ("sleep2", "time between request (us), to control speed", cxxopts::value<uint32_t>()->default_value(std::to_string(200)), "N")
...
void bw_write_bandwidth(uint32_t size, uint32_t interval, uint32_t N,
		uint32_t sleep1, uint32_t sleep2){
...
int main(int argc, char* argv[]){
  ...
  uint32_t sleep1 = allparams["sleep1"].as<uint32_t>();
  uint32_t sleep2 = allparams["sleep2"].as<uint32_t>();
  ...
  if(allparams.count("write")){
    ...
    bw_write_bandwidth(totalsize,interval,N, sleep1, sleep2);
  } else {
    printf("Deprecated\n");
