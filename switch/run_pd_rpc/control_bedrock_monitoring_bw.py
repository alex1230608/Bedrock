...
def cleaning_req_process(msg):
    ...
    for m in msg:
        if m.md_digest_type == 1:
            ...
            # print "Ban dqpn: %d, ip: 0x%s, cmin_win{0-3}: %d %d %d %d" % (
            #     m.ib_bth_dqpn,
            #     hex(m.ipv4_dstAddr),
            #     m.md_cmin_win0,
            #     m.md_cmin_win1,
            ...
            #     m.md_cmin_win3)
            if startBanning:
                print "Ban user_id: %d, cmin_win{0-3}: %d %d %d %d; %d %d %d %d" % (
                    m.md_user_id,
                    m.md_cmin_win0,
                    m.md_cmin_win1,
                    m.md_cmin_win2,
                    m.md_cmin_win3,
                    m.md_cmin_win0_32_20,
                    m.md_cmin_win1_32_20,
                    m.md_cmin_win2_32_20,
                    m.md_cmin_win3_32_20)

                ban_acl_ms = p4_pd.ban_acl_match_spec_t(m.md_user_id)
                try:
...
raw_input("Press Enter to start banning violating users")
startBanning = True
clean_t.join()
