...
public:
  ...
  struct ibv_mr * reg_mem(void *buf, uint64_t size){
      return ibv_reg_mr(this->pd, buf, size, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ);
  }

  struct ibv_mr * reg_mem_readOnly(void *buf, uint64_t size){
      return ibv_reg_mr(this->pd, buf, size, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ);
  }

  struct ibv_mr * reg_mem_writeOnly(void *buf, uint64_t size){
      return ibv_reg_mr(this->pd, buf, size, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE);
  }

  struct ibv_mr * reg_mem_with_atomic(void *buf, uint64_t size){
  ...
  struct ibv_mr * reg_mem_for_mw(void *buf, uint64_t size){
      return ibv_reg_mr(this->pd, buf, size, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_MW_BIND);
  }
  ...
  inline int send_signaled(uint64_t wr_id, uint64_t local_addr, uint32_t lkey, uint32_t length){
    // printf("wr_id: %lu, local_addr: %lu, lkey: %u, length: %u\n", wr_id, local_addr, lkey, length);
